...
**Least privilege** means each key can do only what it actually needs. If a service just reads from one bucket, its key should not be able to delete your whole account. Prefer scoped tokens over all-powerful ones, issue a separate credential per service rather than sharing one everywhere, and you contain the damage when, not if, one leaks. **Rotation** means secrets are not forever. Rotate them on a schedule, and rotate immediately on any trigger: a suspected exposure, a laptop lost, or a team member leaving, since offboarding without rotating shared secrets leaves a door open behind them. The practical enabler for both is that your secrets must be easy to change. If rotating a key means hunting through code and redeploying by hand, you will avoid doing it; if it means updating one environment variable, you will actually rotate when you should. Build for rotation and you will use it.

## Where Kloudbean fits, honestly

The platform gives you the right home for application secrets and revocable access. You set environment variables in the Kloudbean console rather than baking them into your code, so your database URLs, API keys, and tokens live on the server and are easy to update, which is exactly what makes rotation painless. For programmatic access to the platform itself, you issue scoped personal API tokens that you can revoke, rather than one master credential, which is least privilege applied to your own automation. And a self-hosted Vaultwarden gives your team a vault for the human side.

The honest boundary: the platform provides the mechanisms, environment variables set outside your code, revocable scoped tokens, a place to run a vault. It cannot stop you committing a key to git, choosing an over-powerful token, or skipping a rotation, because those are habits, not features. Secrets management is one of the areas where the tooling is the easy half and the discipline is the real work. Use the mechanisms, build the habits, and the keys to your system stay keys, not liabilities. The wider picture of what the platform secures and what stays yours is in [the secure and compliant hosting guide](https://www.kloudbean.com/blog/secure-compliant-hosting/).
