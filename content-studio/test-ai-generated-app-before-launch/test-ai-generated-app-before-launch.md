...
So if you use AI to help write tests, and it's a reasonable way to save time, do two things. Write the test's intent yourself first ("a logged-out user must get 401 from /api/account"), then let the AI fill in the mechanics. And actually read the assertions, which is less daunting than it sounds once you can [read the code AI wrote well enough to follow it](https://www.kloudbean.com/blog/understand-the-code-ai-wrote/). If a test asserts `status === 200` on a route that should reject anonymous users, the test is wrong, no matter how green it is. Test the requirement, not the code's current behaviour.

## Test where it will actually run

The last gap is environmental. "Works on my machine" and "works in production" are different claims, and the difference has ended more launches than any logic bug. Your laptop has your env vars, your local database, your installed tools, and a network that never fails. Production has none of those unless you set them up, which is exactly why an [AI app works locally but not in production](https://www.kloudbean.com/blog/why-my-ai-app-works-locally-but-not-in-production/). With Bolt the gap is wider still, because that preview ran inside your browser tab and never on a server, so [getting a Bolt.new app onto real hosting](https://www.kloudbean.com/blog/deploy-bolt-new-app/) is a step of its own before you can test anything properly.

The fix is to test somewhere production-like before you point real traffic at it. A staging environment with a real managed database turns "works locally" into "works where it's going to run". You catch the missing env var, the local-only shortcut, and the database that behaves differently under a real connection, all before a user ever sees them. One caveat on how you build that: Kloudbean's one-click staging covers WordPress and Laravel specifically, so for a Node or Python app you make staging the honest way, as a second application with its own environment variables and its own database, gated with a [Basic Auth](https://www.kloudbean.com/blog/basic-auth-gate-guide/) prompt so it stays out of search results. Slightly more setup, same benefit. For the wider list of what tends to break at this boundary, [why AI apps fail in production](https://www.kloudbean.com/blog/why-ai-apps-fail-in-production/) is a useful read, and the whole topic sits under the [last mile of vibe coding](https://www.kloudbean.com/blog/last-mile-of-vibe-coding/).
