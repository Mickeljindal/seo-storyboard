...
## Assets
- **Bespoke inline SVG:** a single left-to-right request path (browser HTTPS :443 -> reverse proxy TLS -> Fastify on 0.0.0.0:PORT with pino + graceful shutdown -> managed DB on the private network), with a highlighted fork at the bottom contrasting `listen({ port })` = 127.0.0.1 = 502 vs `listen({ port, host: '0.0.0.0' })` = reachable. Brand navy #000f27 / purple #4F1AF3 / green #40b75f, plus a muted warning stroke on the wrong-host box. Distinct from the Express two-band diagram and the NestJS dev-vs-prod lanes.
