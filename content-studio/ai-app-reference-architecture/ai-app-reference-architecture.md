...
Worth being concrete about what "always-on" means when you go shopping, since the word gets used loosely. You want a process that keeps running whether or not a request arrived in the last ten minutes, which is what a Kloudbean Node or Python app is by default rather than by configuration. There's no warm-up trick to buy and no keep-alive ping to schedule, because nothing went to sleep.
...
Go back to the diagram and look at the dashed line. It isn't a decoration, and it isn't drawn around "our stuff" versus "their stuff." Everything inside shares three properties: it's stateful, it's long-lived, and it talks to its neighbours constantly over short, chatty connections. The one thing outside is a single request over the public internet to a service you don't operate.

That's the engineering reason the layer sits where it does, and it quietly explains most of the advice above. Chatty stateful components want to be close to each other: same account, same region, ideally the same provisioning decision. A connection pool only helps if the database is a couple of milliseconds away and its connection ceiling is a number you actually know. A queue only helps if the worker draining it stays alive. A cache only helps if reaching it is genuinely cheaper than reaching the thing it's caching. Scatter those four across three providers and you've added round trips, three failure domains, and three consoles to precisely the part of the system that most needed to be dull and predictable.

The model provider sits outside for the mirror-image reason. It's one call, it's stateless from your side, its latency is not yours to fix, and its bad days aren't announced in advance. So you wrap it defensively with a timeout, a retry, and a fallback, and you never let it anywhere near your key. That asymmetry is the whole architecture in one sentence: inside the boundary you optimise for proximity and durability, outside it you optimise for distrust.

Which is also the design argument for running the inside of the box in one place, and why the platform kept coming up in the API, state, worker, and region sections instead of arriving here as a surprise. The model itself stays your choice either way. Call a hosted API if that suits you, or self-host an open model on a GPU server when you'd rather it ran inside your own boundary, with DeepSeek available one-click and other models installed on request. What that actually costs to operate is its own decision, worked through in [self-host an LLM](https://www.kloudbean.com/blog/self-host-an-llm/).
