...
| Setup and ops effort | Low, it's an API call | Moderate: you provision the GPU, own sizing and scaling |
...
No, not usually. Self-hosting an open model keeps the prompt in-Kingdom, but it's a real build: you own the GPU sizing, the scaling, and the cost of always-on compute. Most teams keep calling a hosted API and just minimize what they send. Self-host only when the data genuinely cannot leave the Kingdom.
