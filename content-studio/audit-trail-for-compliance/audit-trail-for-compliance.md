...
Which is awkward, because immutability isn't something you can add to a log file in application code. It's a property of the storage underneath, and it has to be enforced by something your own administrators cannot override. That's why it tends to arrive as an infrastructure capability rather than a feature: Kloudbean's enterprise Audit Trail sits on write-once storage with a retention lock precisely so a delete request from a privileged account is refused rather than honoured. If you're building this yourself, that bucket-level lock is the part to get right first, before you worry about what you're recording.
...
That distinction between a documented policy and an enforced one is where most self-built trails quietly fail an assessment. A retention setting somebody can edit is a preference. A retention lock is a constraint. The enterprise Audit Trail on Kloudbean holds account-wide activity on the order of the eighteen-month bar frameworks tend to expect, with the lock doing the enforcing rather than a runbook nobody reads.
...
Saying it is one thing. Handing over the evidence in a form the assessor can actually work with is the other, and it's the step people underestimate. An assessor wants a searchable record and an export they can file, per control, per period. That's why the enterprise trail is searchable account-wide and exports to CSV: the point isn't the storage, it's that the evidence-gathering step doesn't turn into a week of somebody grepping server logs by hand. Worth being exact about what this does and doesn't mean, though. The platform is built to align with and support frameworks like SOC 2 and ISO 27001. It is not certified for them, and no hosting choice certifies your organisation.

## You need two audit trails, and only one of them can be bought

The thing to take away from all of the above is that "audit trail" names two different records living at two different layers, and an assessor will ask about both. Confusing them is how organisations arrive at an audit believing they're covered.

**The infrastructure and account trail** records what happened to the platform: who signed in, who failed to, which permissions changed, which firewall rule moved, which server was reconfigured. This one is genuinely a hosting capability, because immutability and retention locks live in the storage layer rather than in anybody's application code. On Kloudbean that's the enterprise Audit Trail, immutable and searchable across the account with CSV export, on write-once storage with long retention. It's part of the Enterprise package rather than a toggle on a small self-serve plan, and that's an honest scope statement rather than an upsell: tamper-proof long-retained logging is an infrastructure commitment with real cost behind it, so nobody should expect it bundled into an $8 plan.

**The application trail** records what your users did inside your software: which record was viewed, which invoice was approved, which export was downloaded. Nobody can sell you this one. Your host has no idea what your domain objects are or which of your actions are significant, so this trail is code you write, to the same standard as the one above, which means writing it somewhere your own application cannot later edit.
