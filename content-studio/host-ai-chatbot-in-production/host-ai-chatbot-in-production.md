...
Redis earns its place separately. It's where sessions live, where rate-limit counters tick, and where you cache repeated answers so an identical question doesn't pay for a fresh model call. Set sensible TTLs so it stays small. See [managed Redis hosting](https://www.kloudbean.com/blog/managed-redis-hosting/) for the setup. And because every chat request opens a database connection, put a pool in front of Postgres or you'll exhaust connections under load. [Database connection pooling](https://www.kloudbean.com/blog/database-connection-pooling/) explains why that limit bites sooner than people expect.
...
- **Keep the key server-side.** Load it from an environment variable, never inline it in client code. [Secrets management](https://www.kloudbean.com/blog/secrets-management/) and [environment variables done right](https://www.kloudbean.com/blog/environment-variables-done-right/) cover the how.
