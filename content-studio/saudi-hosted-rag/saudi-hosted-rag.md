...
1. **Where does the file arrive?** The upload endpoint, then the bucket behind it. Write down the bucket's region, not the app's.
2. **Where does the text get extracted?** A PDF parser or OCR step is a hop. If it's a third-party API, that's a crossing nobody drew on the diagram.
3. **Where is each chunk embedded?** The most-missed hop on this list. A hosted embeddings API means every document already left, at index time and again on every re-index.
4. **Where do the vectors land?** The database region. Then check the backup region separately, because they are not automatically the same thing.
5. **Where is the question embedded?** Same model as hop 3, so the same answer, except now it applies to whatever a user typed.
6. **Where does the similarity search run?** Inside the database. This hop never leaves, which is the good news in the whole trace.
7. **Where does the retrieved context go?** The generation call. Log the actual payload once, top-k and all. Most teams are surprised by the volume of their own documents on that wire.
8. **Where do the logs and traces go?** Error trackers and observability tools capture request bodies by default. This is the leak that survives every other fix.

Hops 1, 2, 4 and 6 are infrastructure, and one region choice settles them. Google Cloud's Dammam region, `me-central2`, holds the always-on Node or Python server, the managed Postgres carrying chunks and embeddings through `pgvector` where available, and the S3-compatible object storage for source files, with no egress fees on the built-in storage you pull back for indexing. Automatic backups stay in-region, SSL is free, deploys come from Git, and it's one dashboard rather than four consoles in three countries. The region specifics are in [the GCP Dammam region guide](https://www.kloudbean.com/blog/gcp-dammam-region-guide/).
