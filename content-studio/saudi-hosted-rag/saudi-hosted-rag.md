...
No. Kloudbean gives you the in-Kingdom home for the documents, the embeddings, and the retrieval: object storage, a managed Postgres with pgvector, and always-on compute in the Dammam region. It does not sell GPUs or model inference, so the generation model is your choice, a hosted API with minimized context, or an open model you self-host on suitable compute. The resident half of the stack is what Kloudbean runs for you.
