...
One thing that shrinks the map: keep those stores in one place and in one country. Running the app, its managed Postgres holding the embeddings (through `pgvector`, where the Postgres version you choose makes it available), the Redis cache, and object storage in a single account in the Dammam region, `me-central2`, means the inventory above is five tables in one jurisdiction rather than five vendors in four. That does not reduce your obligations. It reduces how many places you have to reach to meet them.
...
What you need for that row is a documented, boring backup cycle you can describe in a sentence. Kloudbean's automatic backups run on a schedule and stay in-region when your server is in Dammam, and there are on-demand backups too, so you can state where copies live and how long they persist without reconstructing it from memory during a request.
...
The moment your app calls a hosted model like OpenAI, Claude, or Gemini, the prompt leaves the Kingdom. PDPL regulates the cross-border transfer of personal data. It does not ban it. So this isn't forbidden, it's an obligation to manage: know what's in the payload, send less, and understand the provider's terms. There is also a way to remove the transfer instead of managing it: run the model in the Kingdom yourself. GPU servers are self-serve on Kloudbean, DeepSeek and Open WebUI install in one click, and other or custom models get installed by the support team on request, so inference can sit in Dammam beside the data and the prompt never leaves. That's a real build with its own sizing and cost tradeoffs, weighed in [self-host an LLM](https://www.kloudbean.com/blog/self-host-an-llm/). The full data-flow map, and how to shrink what crosses, is the sibling piece on [what happens when you call OpenAI, Claude, or Gemini from Saudi Arabia](https://www.kloudbean.com/blog/openai-claude-gemini-saudi-data/). The hosting and residency half is in [PDPL compliance hosting](https://www.kloudbean.com/blog/pdpl-compliance-hosting/). I won't rebuild either here.

Then automated decisions. If your AI makes or heavily shapes decisions about people, say approving an application, scoring a candidate, or flagging an account, privacy regimes tend to pay special attention. The legal test is for an advisor. The engineering hooks are simple, and worth building whether or not they're strictly required for your case: keep a human-review path so a person can check and override the model, and keep an audit record of what the model decided and on what input. If you can't reconstruct why user 8842 was rejected, you can't defend it and you can't fix it. Note the split here: the decision record is application data you have to write yourself, while the separate question of who touched the infrastructure is what an account-wide audit trail with CSV export covers, and that one is an Enterprise capability rather than a default.

## Seven questions you'll be asked, and where each answer has to come from

When a data-subject request lands, or your DPO starts a review, or a customer's procurement team sends a questionnaire, it arrives as questions. Here they are, with the honest source of each answer. Sort your own list this way and you'll see instantly which gaps are yours to close.

- **What personal data do you hold about this person, and where?** Answer comes from your own data map. Nobody can produce it for you, and if the map does not exist, the map is the first task.
- **Show me the deletion actually completing across every store.** Answer comes from your code: the query or job that clears rows, messages, vectors, and cache keys by user id, plus a log of it running.
- **What did this user consent to, and when?** Answer comes from your consent records, stored as data with the notice version and timestamp. A boolean cannot answer this.
- **How long do you keep chat logs and embeddings, and what enforces it?** Answer comes from your retention job. A policy document without a cron entry behind it answers nothing.
- **What is your lawful basis for this processing?** Answer comes from your qualified Saudi data-protection advisor, informed by how you actually built the thing.
- **Where does the data physically sit, and where do backups live?** Answer comes from your infrastructure. Running in Google Cloud's Dammam region, `me-central2`, with the app, its managed database, object storage, and automatic backups in one in-Kingdom account, makes this a one-line answer.
- **Who can reach the database, and how is that restricted?** Also infrastructure. On a standard plan the mechanism is IP allow-listing: you whitelist your app server's IP on the managed database so only that server connects, paired with strong credentials and free SSL. Network isolation in a private VPC is an Enterprise capability, not a default, so describe what you actually have.

Five of those seven answers come out of your codebase or your advisor. No host answers them, ours included. Kloudbean will not write your deletion logic, record your consent, set your retention windows, or choose your lawful basis, and a provider that implies otherwise is selling you a gap you'll discover under a deadline.
