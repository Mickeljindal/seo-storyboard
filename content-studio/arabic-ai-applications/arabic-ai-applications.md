...
- **Text renders as `?????` or `Ø§Ù„`.** Character set, every time, and check the connection charset before you touch the column. If Postgres is your database and you're still seeing it, look further up the path than the database.
- **A number, URL, or brand name lands at the wrong end of the line.** Bidi, not fonts, not the model. `dir` on the container and `<bdi>` around embedded Latin runs fixes the large majority of it.
- **Retrieval misses documents a human would call an obvious match.** Normalization, and usually because it was applied on one side only. Same function on documents and queries, or it's worse than none.
