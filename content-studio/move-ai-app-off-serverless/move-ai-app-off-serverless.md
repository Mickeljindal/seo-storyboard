...
2. **Put the database behind a connection pool.** This is the fix for the connection storm. Instead of opening a client per request, open one pool when the process starts and let every request borrow from it. Size it well under the database's limit. If the database is also moving, provision it as its own managed service (Postgres, MySQL and MariaDB are three of the seven engines Kloudbean manages) and whitelist your app server's IP on it, so exactly one host can connect. Our [database connection pooling](https://www.kloudbean.com/blog/database-connection-pooling/) guide covers sizing, but the shape is small:
