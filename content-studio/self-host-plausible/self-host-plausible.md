...
Plausible CE is distributed to run with its ClickHouse and PostgreSQL dependencies, so a self-host is the Plausible app, a ClickHouse instance for events, and Postgres for configuration, behind a reverse proxy with HTTPS. ClickHouse is the memory-hungry part, so give the server room. On the Postgres side, pointing the configuration database at a [managed PostgreSQL](https://www.kloudbean.com/blog/managed-postgresql-hosting/) takes one component off your plate and keeps it backed up, while ClickHouse runs on the server as part of Plausible's own stack.
