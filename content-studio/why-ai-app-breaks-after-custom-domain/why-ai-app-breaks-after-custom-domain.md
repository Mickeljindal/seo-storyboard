...
Which is why it matters whether you can see your builds. On Kloudbean the variable is a field in the app's settings and a push triggers managed CI/CD, with live build logs and deployment history, so you can confirm a new bundle actually compiled with the new value instead of assuming it did. That assumption is where people lose an hour: they change the variable, the app restarts, the old URL is still in the network tab, and they conclude the variable is broken. It isn't. The bundle just predates it.
...
A "not ready yet" error clears on its own within minutes as DNS resolves and the certificate issues. A misconfiguration doesn't clear no matter how long you wait: the name still resolves nowhere, or a stray record blocks issuance. Where the certificate is issued for you, this is one less thing to get wrong. Add the domain on Kloudbean and free SSL is provisioned and then auto-renewed, which also quietly removes the expiry outage a year from now, the one nobody has in their calendar. Give it a reasonable wait first. If a certificate warning is still there well after that, it's config, and [fixing SSL certificate errors](https://www.kloudbean.com/blog/fix-ssl-certificate-errors/) walks the causes. For the ordering of the whole DNS-then-certificate setup in the first place, see [adding a custom domain and free SSL to your app](https://www.kloudbean.com/blog/custom-domain-and-ssl-for-your-app/).
...
## Why waiting for DNS to propagate cannot fix most of this

The reflex, when a domain switch goes wrong, is to wait. Clear the cache, try incognito, give propagation an hour, ask in a Discord whether it's still propagating. Sometimes that's genuinely right, and I said as much a moment ago about an unissued certificate. For most of the failures on this page, waiting cannot help at all, and understanding why is what stops you burning the afternoon on it.

DNS does one job. It maps a name to an IP address. Propagation finishing only means more resolvers now know the new answer. It says nothing about what your bundle was compiled with, which origin your API trusts, which host a cookie is scoped to, or which callback URL sits in Google's records. A CORS rejection is a decision your own API made about a string in its own config, in a couple of milliseconds, with DNS resolved long before. A hardcoded preview URL is a literal inside a compiled file, and time doesn't edit files. A redirect URI is a row in someone else's database that only you can change.

So turn the reflex into a diagnostic. Two categories, and every symptom belongs to exactly one:

- **Time actually fixes it.** The name doesn't resolve yet, or the certificate hasn't been issued yet. Both are "not ready", and both clear on their own, usually within minutes. No error message names anything specific.
- **Time cannot fix it.** Anything that produces a precise complaint: a CORS message naming two origins, `redirect_uri_mismatch`, a request to the old hostname sitting in your network tab, a silent logout, `ERR_TOO_MANY_REDIRECTS`. Each one is a stored string, and somebody has to go change it.

That split is also the honest line between what a platform does and what you do. The first category is the part Kloudbean takes: add the domain, free SSL is issued and auto-renews, environment variables live in the app's settings, and a Git push rebuilds with live build logs so you can see the new value compiled in. Deploys are from $8/mo and it's one dashboard, not three.
