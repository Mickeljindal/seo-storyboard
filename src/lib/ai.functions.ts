...
- LOGS ARE IN THE UI. Lead with the Logs Viewer, not a file path or SSH. Application Administration then Logs Viewer, with logs grouped into tabs by type: Web Requests Logs (web server access logs, every request served), App Info (app.info.log), and App Errors (app.error.log, for diagnosing crashes and 503s). There is a built-in search to filter logs and locate a specific error. Build and deployment logs stream live and are kept in Build and Deployment History. The same files are ALSO readable via the File Manager at /home/admin/hosted-sites/<app_system_user>/app-logs, but that is the secondary route, so never present File Manager or SSH as the only way or the first way to read logs. Documented 503 diagnostic worth using: if the site returns a 503, check the application logs, because it means the application is not running.
