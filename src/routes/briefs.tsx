...
function Briefs() {
  ...
  const bulk = useMutation({
    ...
    onSuccess: (r: any) => {
      toast.success(`Generated ${r.success}/${r.total} briefs${r.failed ? ` · ${r.failed} failed` : ""}`);
      ...
      <div className="mx-auto max-w-6xl px-8 py-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{articles.length} of {total} ready · {missing} missing</p>
          </div>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button disabled={bulk.isPending || missing === 0}>
              ...
              <AlertDialogHeader>
                ...
                <AlertDialogDescription asChild>
                  <div className="space-y-2 text-sm">
                    <p>This calls your configured OpenAI-compatible model once per article, up to 3 in parallel.</p>
                    <ul className="ml-4 list-disc text-muted-foreground">
                      <li><b>~{missing} AI calls</b> against your API key</li>
                      <li><b>Estimated cost: ~${estCost}</b> (model-dependent)</li>
                      <li><b>Estimated time: ~{estMinutes} min</b></li>
                      <li>Existing briefs are skipped — only missing articles run</li>
                    </ul>
                    <p className="text-xs">If you hit a 429 (rate limit) or 402 (out of credits), retry the bulk run later.</p>
                  </div>
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={() => bulk.mutate()}>Generate {missing}</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
        ...
        {(articles?.length ?? 0) === 0 ? (
          <div className="rounded-lg border border-dashed border-border bg-card p-12 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">No briefs yet. Open any article in the Calendar and click "Generate Brief".</p>
          </div>
        ) : (
          <div className="grid gap-3 md:grid-cols-2">
            {articles!.map((a) => {
              const b: any = a.brief;
              return (
                <button key={a.id} onClick={() => { setActiveId(a.id); setOpen(true); }} className="rounded-lg border border-border bg-card p-4 text-left transition-colors hover:border-primary/50">
                  <div className="mb-2 flex items-center gap-2"><PillarBadge pillar={a.pillar} /><StatusBadge status={a.status} /></div>
                  <div className="font-medium">{b?.h1 ?? a.title}</div>
                  <div className="mt-1 text-xs text-muted-foreground line-clamp-2">{b?.meta_description ?? a.target_keyword}</div>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {(b?.secondary_keywords ?? []).slice(0, 4).map((k: string) => (
                      <span key={k} className="rounded bg-secondary px-1.5 py-0.5 text-[10px]">{k}</span>
                    ))}
                  </div>
                </button>
              );
