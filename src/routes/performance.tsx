...
import {
  TrendingUp, MousePointerClick, Eye, MapPin, Loader2, RefreshCw,
  CheckCircle2, AlertCircle, Link2,
} from "lucide-react";
import { toast } from "sonner";
import {
  getAnalyticsDashboardFn, syncSearchConsoleFn, testGscConnectionFn,
...
function Performance() {
  ...
  const sync = useMutation({
    ...
    onSuccess: (r: { ok: boolean; stored?: number; matched?: number; window?: string; error?: string }) => {
      if (r.ok) {
    ...
    <AppLayout>
      <div className="mx-auto max-w-[1200px] px-8 py-8">
        <header className="mb-8">
          ...
          <h1 className="text-display text-3xl font-semibold tracking-tight">Real search performance</h1>
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            First-party ranking data pulled straight from Google Search Console — no paid third-party tool. Real clicks,
            impressions, and positions feed the self-learning ranker, so discovery prioritises the clusters that
            actually win.
          </p>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <Button variant="outline" size="sm" disabled={test.isPending} onClick={() => test.mutate()}>
              {test.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Link2 className="mr-2 h-4 w-4" />}
              Test connection
            </Button>
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium ${
                configured ? "bg-[var(--lime)]/15 text-[var(--lime)]" : "bg-secondary text-muted-foreground"
              }`}
            >
        ...
        {!configured && (
          <div className="mb-8 rounded-xl border border-border bg-card/60 p-6">
            ...
            <ol className="mt-3 list-decimal space-y-1.5 pl-5 text-xs text-muted-foreground">
              ...
              <li>Search Console → Settings → Users and permissions → add the service account email (Restricted is fine).</li>
              <li>
        ...
        <div className="mb-8 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-border bg-border md:grid-cols-4">
          <Stat icon={MousePointerClick} label="Clicks" value={hasData ? totals.clicks.toLocaleString() : "—"} accent />
          <Stat icon={Eye} label="Impressions" value={hasData ? totals.impressions.toLocaleString() : "—"} />
                    ...
                    <tr key={p.page} className="border-b border-border/50 last:border-0">
                      <td className="py-2.5 pr-3">
                        <div className="flex items-center gap-2">
                          {p.articleId && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-primary" title="Matched to an article" />}
                          <a href={p.page} target="_blank" rel="noreferrer" className="line-clamp-1 max-w-[420px] hover:text-primary">
                            {p.page.replace(/^https?:\/\//, "")}
                          </a>
                        </div>
                      </td>
                      <td className="num py-2.5 px-3 text-right font-medium">{p.clicks.toLocaleString()}</td>
                      <td className="num py-2.5 px-3 text-right text-muted-foreground">{p.impressions.toLocaleString()}</td>
                      <td className="num py-2.5 px-3 text-right text-muted-foreground">{(p.ctr * 100).toFixed(1)}%</td>
                      <td className="num py-2.5 pl-3 text-right">
                        <span className={p.position <= 3 ? "text-[var(--lime)]" : p.position <= 10 ? "text-foreground" : "text-muted-foreground"}>
        ...
        <footer className="mt-12 flex items-center justify-center gap-2 text-center font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
          <TrendingUp className="h-3 w-3" /> Real outcomes → self-learning ranker · the long-term moat
        </footer>
      </div>
    </AppLayout>
  );
