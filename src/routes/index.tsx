...
function Dashboard() {
  ...
  const seedMut = useMutation({
    ...
    onSuccess: (r: any) => {
      toast.success(
        r.skipped ? `Already have ${r.existing} articles` : `Seeded ${r.inserted} articles`,
      );
      refetch();
    },
  ...
  const stats = useMemo(() => {
    ...
    const published =
      articles?.filter((a) => a.status === "published" || a.status === "promoted").length ?? 0;
      ...
      <div className="mx-auto max-w-[1280px] px-10 py-10">
        {isError && (
          <div className="mb-6 rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-sm text-destructive">
            ...
            <code className="font-mono text-xs">npm run dev</code> (or use{" "}
            <code className="font-mono text-xs">npm run dev:local</code>).
          </div>
        )}
        {/* HERO — mission */}
        <header className="mb-12 grid grid-cols-12 gap-6">
          <div className="col-span-12 lg:col-span-8">
            ...
            <p className="mt-5 max-w-xl text-[15px] leading-relaxed text-muted-foreground">
              10 topical-authority clusters built for Kloudbean's real buyers — vibecoders deploying
              Lovable & Bolt apps, agencies hosting many client sites, SaaS founders self-hosting
              n8n & Supabase, and enterprises that need data residency. Spin the{" "}
              <span className="font-mono text-foreground">Idea Raffle</span>, generate the brief,
              auto-publish to <span className="font-mono text-foreground">kloudbean.com</span> — and
              win Google + AI search without waiting for backlinks.
            </p>
            <div className="mt-6 flex flex-wrap items-center gap-3">
              {empty ? (
                <Button
                  ...
                  size="lg"
                  title="Create a starter plan of 59 article ideas so the engine has topics to work on."
                >
                ...
                <Button
                  ...
                  asChild
                  title="Open the engine page to start the automatic find-write-publish pipeline."
                >
                  <Link to="/engine">
              ...
              <Button
                ...
                className="gap-2"
                asChild
                title="Open Settings to connect WordPress, AI, and search-data accounts."
              >
                <Link to="/settings">
              ...
              <div className="relative">
                ...
                <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                  <div
                    ...
                    style={{ width: `${stats.pct}%`, background: "var(--gradient-brand)" }}
                  />
                </div>
        ...
        <section className="mb-12">
          <SectionLabel
            kicker="01"
            title="The pipeline"
            desc="Every article moves through these seven stages — automated by the engine."
          />
        ...
        <section className="mb-12">
          <SectionLabel
            ...
            desc="Live counters from your content roadmap."
          />
            ...
            <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4">
              {Array.from({ length: 12 }).map((_, i) => {
                ...
                return (
                  <div
                    key={week}
                    className="rounded-lg border border-border bg-card/60 p-3 backdrop-blur transition hover:border-primary/40"
                  >
                    ...
                    <div className="mt-2 space-y-1">
                      {items.slice(0, 3).map((a) => (
                        <button
                          ...
                          className="flex w-full items-start gap-1.5 rounded px-1 py-1 text-left text-[11px] leading-tight hover:bg-foreground/5"
                        >
            ...
            <div className="mt-6 space-y-1.5">
              {STATUSES.map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className="rounded-md border border-border bg-card/60 px-3 py-2 backdrop-blur"
                  >
                    ...
                    <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-secondary">
                      <div
                        className="h-full rounded-full"
                        style={{ width: `${pct}%`, background: s.tokenVar }}
                      />
                    </div>
                  </div>
                );
        ...
        <footer className="mt-16 flex items-center justify-between border-t border-border pt-6 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
          ...
          <span>
            10 clusters · Kloudbean-only topics · deploy · compliance · self-hosted on Kloudbean
          </span>
        </footer>
      </div>
  ...
  return (
    <div className="bg-card/80 p-5">
      ...
      <div className="mt-3 flex items-baseline gap-1.5">
        <span className={`num text-display text-3xl font-semibold ${accent ? "grad-text" : ""}`}>
          {value}
        </span>
